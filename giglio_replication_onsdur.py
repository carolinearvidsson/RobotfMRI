...
from giglio_conversations import Conversations
...
class OnsetsDurations: 
    def __init__(self, path):
        ...
        tdl_file = open("pickles/tdl.pkl", "wb")
        pickle.dump(self.onsdurs_output, tdl_file)
        tdl_file.close()

        #self.collapsed = self.collapse_conditions(self.onsdurs_output, [['PAUSE_c_h'], ['PAUSE_p_h'], ['GAP_p2c_h'], ['GAP_c2p_h'], ['PAUSE_c_r'], ['PAUSE_p_r'], ['GAP_p2c_r'], ['GAP_c2p_r']], ['SILENCE'])
        # self.final_output = self.collapse_conditions(self.collapsed, [['PAUSE_c_r'], ['PAUSE_p_r'], ['GAP_p2c_r'], ['GAP_c2p_r']], ['SILENCE_r'])
        self.final_output = self.crop_duration(self.onsdurs_output) #remove events <300 ms
...
### REMOVE REST AND ADD PROD H COMPH PROD R COMP R
                    ...
                    names = (comp_1_name, comp_3_name, \
                                        prod_1_name, prod_3_name, \
                                                    comp_rest_name, prod_rest_name)
                    
                    onsets = (self.comp_1_onsets, self.comp_3_onsets, \
                                        self.prod_1_onsets, self.prod_3_onsets, \
                                                    self.comp_rest_onsets, self.prod_rest_onsets)


                    durations = (self.comp_1_durs, self.comp_3_durs, \
                                        self.prod_1_durs, self.prod_3_durs, \
                    ...
                    silence_name, self.silence_onsets, self.silence_durs = 'SILENCE', [], []

                    names = [silence_name]

                    onsets = [self.silence_onsets]

                    durations = [self.silence_durs]
    ...
    def append_transition_parameters(self, row, onset, duration, conv_onset, condition, SubjRunID, n_token):
        ...
        if condition == 'human':
            if self.check_transition(row) == 'GAP_p2c':
                self.silence_onsets.append(new_onset)
                self.silence_durs.append(duration)
                if self.pmod == True:
